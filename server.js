...
app.get('/api/inicio/all', async function (req, res) {
    ...
    var articles = await sql.query`SELECT Nombre, ImgUrl, FORMAT(Fecha,'yyyy-MM-dd HH:mm:ss') as Fecha FROM Articles ORDER BY cast([Fecha] as datetime) DESC  `;
...
app.get('/api/inicio/all/:next', async function (req, res) {
    ...
    var articles = await sql.query`SELECT Nombre, ImgUrl, FORMAT(Fecha,'yyyy-MM-dd HH:mm:ss') as Fecha FROM Articles ORDER BY cast([Fecha] as datetime) DESC OFFSET 9 ROWS FETCH NEXT ${next} ROWS ONLY `;
    var result = JSON.parse(JSON.stringify(articles))
    res.json({ data: result });
...
app.get('/api/articulo/:art', async function (req, res) {
    ...
    var section = await sql.query`SELECT Nombre, IdSection from Sections`;
    var PostSection = req.params.section;
    ...
    WHERE a.Url = ${ImgUrl}`
    var result2 = await sql.query`select top 4 Url, Nombre, ImgUrl  from Articles WHERE Url != ${ImgUrl} Order By NEWID()`
    var resultArray = [section]
